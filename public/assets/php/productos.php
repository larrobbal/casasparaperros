<?php
    ...
    else if(isset($response['idProducto']) || isset($_POST['idProducto']))
    {
        ...
        $query="SELECT descripcion,linea,acabado,material,calibre,capacidad,colores,anclaje,vaciado,largoTotal,anchoTotal,altoTotal,diametroTotal,largoCont,anchoCont,altoCont,diametroCont,largoLetrero,altoLetrero,adicional,imagen FROM descripcion_producto where id_descripcion = '$idProducto'";
        $result_query = $data_source->exeConsulta($query);
        foreach($result_query as $row)
        {
